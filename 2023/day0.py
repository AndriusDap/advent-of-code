...
for line in values.split():
    first = min(numbers, key = lambda x: line.find(x) if x in line else 1000)
    last = max(numbers, key = lambda x: line.find(x) if x in line else 0)
    print(decode(first) * 10 + decode(last))
    sum = sum + (decode(first) * 10 + decode(last))
